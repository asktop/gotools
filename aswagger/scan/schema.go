...
func (scp *schemaParser) parseNamedType(gofile *ast.File, expr ast.Expr, prop swaggerTypable) error {
	...
	case *ast.Ident: // simple value
		...
		return scp.parseIdentProperty(gofile, pkg, ftpe, prop)
...
func (scp *schemaParser) packageForFile(gofile *ast.File, tpe *ast.Ident) (*loader.PackageInfo, error) {
	...
	for pkg, pkgInfo := range scp.program.AllPackages {
		if Debug && gofile.Name.Name == "atvert" {
			log.Println("inferring for", tpe.Name, "with", gofile.Name.Name, "at", pkg.Path(), "against", filepath.ToSlash(fgp))
			log.Println("pkg.Name()", pkg.Name(), "gofile.Name.Name", gofile.Name.Name, "filepath.ToSlash(fgp)", filepath.ToSlash(fgp), "pkg.Path()", pkg.Path(), fgp)
		}
...
func (scp *schemaParser) parseIdentProperty(gofile *ast.File, pkg *loader.PackageInfo, expr *ast.Ident, prop swaggerTypable) error {
	...
	if strings.HasSuffix(pkg.String(), `github.com/asktop/decimal`) && expr.String() == "Big" {
		prop.Typed("number", "decimal")
		return nil
	}
	if pkg.String() == `encoding/json` && expr.String() == "Number" {
		prop.Typed("number", "decimal")
		return nil
	}
	/*
case *ast.StructType:
	...
	*/
	...
	//如果是一个子对象,是否要直接翻译成结构体

	if title, ok := title(gd.Doc); ok {
		if at, ok := ts.Type.(*ast.StructType); ok {
			sh := prop.Schema()
			sh.Title = title
			return scp.parseStructType(gofile, sh, at, make(map[string]string))
		}
	}
...
func (scp *schemaParser) typeForSelector(gofile *ast.File, expr *ast.SelectorExpr, prop swaggerTypable) error {
	...
	return scp.parseIdentProperty(gofile, pkg, expr.Sel, prop)
}
...
func title(comments *ast.CommentGroup) (string, bool) {
	if comments != nil {
		for _, cmt := range comments.List {
			for _, ln := range strings.Split(cmt.Text, "\n") {
				matches := rxTitle.FindStringSubmatch(ln)
				if len(matches) > 1 && len(strings.TrimSpace(matches[1])) > 0 {
					return strings.TrimSpace(matches[1]), true
				}
			}
