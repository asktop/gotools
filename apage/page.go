...
const (
    ...
    FORMATUNIXNANOTIMESTAMP = "FORMATUNIXNANOTIMESTAMP" //纳秒级时间戳格式化
    FORMATMILLITIMESTAMP    = "FORMATMILLITIMESTAMP"    //毫秒级时间戳格式化
    FORMATTIMESTAMP         = "FORMATTIMESTAMP"         //秒级时间戳格式化
    FUNC                    = "FUNC"                    //方法处理
    URL                     = "URL"                     //超链接
)
...
//纳秒级时间戳格式化
func (p *Page) FormatUnixNanoTimestamp(name string, format string) *Page {
    key := name + " " + FORMATUNIXNANOTIMESTAMP
    if _, ok := p.execValues[key]; !ok {
        p.execFields = append(p.execFields, key)
    }
...
//毫秒级时间戳格式化
func (p *Page) FormatMilliTimestamp(name string, format string) *Page {
    key := name + " " + FORMATMILLITIMESTAMP
    if _, ok := p.execValues[key]; !ok {
        p.execFields = append(p.execFields, key)
    }
    p.execValues[key] = format
    ...
    for i, data := range p.Data {
        ...
        for _, key := range p.execFields {
            ...
            case FORMATUNIXNANOTIMESTAMP:
                ...
                execVal := exec.(string)
                if value != "" && value != "0" {
                    value = atime.FormatUnixNanoTimestamp(execVal, value)
                } else if value == "0" {
                    value = ""
                }
            case FORMATMILLITIMESTAMP:
                //毫秒级时间格式化
                execVal := exec.(string)
                if value != "" && value != "0" {
                    value = atime.FormatMilliTimestamp(execVal, value)
                } else if value == "0" {
                    value = ""
                }
