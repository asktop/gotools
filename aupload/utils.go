...
func NewAdminFilePathName(adminId int64, typ string) (imgPathName string, thumbPathName string) {
    newFileName := NewFileName()
    newFileName = filepath.Join("admin", acast.ToString(adminId), typ, newFileName)
    newFileName = strings.ReplaceAll(newFileName, `\`, `/`)
    imgPathName = newFileName
    thumbPathName = newFileName + "-thumb"
    return
}

func NewUserFilePathName(userId int64, typ string) (imgPathName string, thumbPathName string) {
    newFileName := NewFileName()
    newFileName = filepath.Join("user", acast.ToString(userId), typ, newFileName)
    newFileName = strings.ReplaceAll(newFileName, `\`, `/`)
    imgPathName = newFileName
    thumbPathName = newFileName + "-thumb"
    return
