...
import (
	"bytes"
	"github.com/asktop/gotools/charset"
	"strings"
	"testing"
)
...
func buildXml(charsetStr string, str string) (string, string) {
	...
	srcXml, err = charset.UTF8To(charsetStr, srcXml)
	if err != nil {
		return "", ""
	}
...
func Test_XmlToJson(t *testing.T) {
	for _, v := range testData {
		...
		if err != nil {
			t.Errorf("ToJson error. %s", srcXml)
		}
...
func Test_Decode(t *testing.T) {
	for _, v := range testData {
		...
		if err != nil {
			t.Errorf("Decode error. %s", srcXml)
		}
